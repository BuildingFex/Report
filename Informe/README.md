...
| **US-04** | Ver planes disponibles | **Como** administrador, **quiero** ver los planes de suscripción **para** evaluar costos. | **Scenario:** Visualización de la tabla de precios.<br>**Given** que el usuario accede a la sección de costos,<br>**When** visualiza los planes Esencial, Estándar y Escala,<br>**Then** identifica claramente el precio mensual de cada uno.<br><br>**Scenario:** Error al cargar la moneda local.<br>**Given** que el usuario accede desde una ubicación no reconocida,<br>**When** el sistema intenta mostrar los precios,<br>**Then** se muestran los precios en dólares americanos (USD) por defecto. | EP-01 |
| **US-05** | Comparar planes | **Como** junta directiva, **quiero** comparar características entre planes **para** elegir el adecuado. | **Scenario:** Comparación de límites y beneficios.<br>**Given** que el usuario visualiza la tabla comparativa,<br>**When** analiza las filas de características,<br>**Then** diferencia los límites de unidades y módulos permitidos por plan.<br><br>**Scenario:** Confusión por términos técnicos.<br>**Given** que el usuario lee una característica compleja,<br>**When** posiciona el cursor sobre el término,<br>**Then** el sistema muestra una ayuda visual (tooltip) explicando el concepto de forma sencilla. | EP-01 |
| **US-06** | Suscribirme | **Como** usuario interesado, **quiero** tener un botón de suscripción **para** iniciar el proceso de compra. | **Scenario:** Inicio exitoso de suscripción.<br>**Given** que el usuario elige un plan,<br>**When** presiona el botón "Suscribirse",<br>**Then** es redirigido al formulario de registro y posteriormente a la pasarela de pago.<br><br>**Scenario:** Intento de suscripción con campos incompletos.<br>**Given** que el usuario está en el formulario de compra,<br>**When** intenta avanzar sin completar los datos de facturación obligatorios,<br>**Then** el sistema resalta los campos vacíos en rojo y bloquea el botón de continuar. | EP-01 |
| **US-07** | Ver beneficios del sistema | **Como** administrador, **quiero** conocer ventajas del sistema **para** confiar en la solución. | **Scenario:** Lectura de ventajas estratégicas.<br>**Given** que el usuario navega en la sección de beneficios,<br>**When** lee sobre la escalabilidad y la unificación de datos,<br>**Then** comprende cómo BuildingFlex optimiza su operación.<br><br>**Scenario:** Iconografía no cargada.<br>**Given** que el usuario visualiza la sección de beneficios,<br>**When** las imágenes de soporte fallan en cargar,<br>**Then** el sistema muestra los títulos y descripciones de texto sin perder la coherencia del contenido. | EP-01 |
| **US-08** | Consultar preguntas frecuentes | **Como** usuario interesado, **quiero** revisar preguntas frecuentes **para** resolver dudas rápidamente. | **Scenario:** Resolución de dudas comunes.<br>**Given** que el usuario accede a la sección FAQ,<br>**When** hace clic en una pregunta sobre seguridad de datos,<br>**Then** la respuesta se despliega de forma clara y concisa.<br><br>**Scenario:** Respuesta no encontrada.<br>**Given** que el usuario revisa todas las preguntas,<br>**When** ninguna resuelve su inquietud particular,<br>**Then** visualiza un enlace directo para contactar al equipo de soporte. | EP-01 |
| **US-09** | Buscar ayuda | **Como** usuario, **quiero** buscar información específica dentro de la sección de ayuda **para** encontrar respuestas precisas. | **Scenario:** Búsqueda efectiva por palabra clave.<br>**Given** que el usuario utiliza la barra de búsqueda en FAQ,<br>**When** escribe "conciliación",<br>**Then** el sistema filtra y muestra solo las preguntas relacionadas con pagos.<br><br>**Scenario:** Búsqueda sin coincidencias.<br>**Given** que el usuario ingresa un término irrelevante o mal escrito,<br>**When** presiona buscar,<br>**Then** el sistema muestra un mensaje indicando que no hay resultados y sugiere términos similares. | EP-01 |
| **US-10** | Navegar por secciones | **Como** usuario, **quiero** usar el menú de navegación **para** moverme fácilmente por la landing. | **Scenario:** Desplazamiento fluido por el menú.<br>**Given** que el usuario está en la parte superior de la página,<br>**When** hace clic en "Beneficios" en el menú,<br>**Then** la página se desplaza suavemente hasta esa sección específica.<br><br>**Scenario:** Menú bloqueado por scroll.<br>**Given** que el usuario navega hacia abajo en la página,<br>**When** intenta usar el menú,<br>**Then** el menú permanece fijo (sticky) en la parte superior para ser accesible en todo momento. | EP-01 |
| **US-11** | Cambiar idioma | **Como** usuario, **quiero** cambiar el idioma **para** entender mejor el contenido de la plataforma. | **Scenario:** Cambio de idioma a inglés.<br>**Given** que la landing está en español,<br>**When** el usuario selecciona el idioma "EN" en el selector,<br>**Then** todo el texto de la interfaz se traduce correctamente al inglés.<br><br>**Scenario:** Persistencia de idioma fallida.<br>**Given** que el usuario cambió el idioma a inglés,<br>**When** refresca la página o navega a otra sección,<br>**Then** el sistema debe mantener el idioma elegido sin volver al español automáticamente. | EP-01 |
| **US-12** | Acceder a la aplicación | **Como** usuario interesado o registrado, **quiero** tener un botón visible **para** acceder a la aplicación web. | **Scenario:** Redirección directa al Login.<br>**Given** que el usuario está en la landing,<br>**When** hace clic en el botón de acceso,<br>**Then** se abre la pantalla de inicio de sesión de la plataforma SaaS.<br><br>**Scenario:** Intento de acceso sin conexión.<br>**Given** que el usuario presiona el botón de acceso,<br>**When** el servidor de la aplicación web está caído o sin respuesta,<br>**Then** la landing muestra un aviso informando que el servicio estará disponible pronto por mantenimiento. | EP-01 |
| **US-13** | Navegación adaptada en móvil | **Como** junta directiva, **quiero** navegar fácilmente desde mi celular **para** acceder rápidamente a la información. | **Scenario:** Uso del menú tipo "hamburguesa".<br>**Given** que el usuario accede desde un smartphone,<br>**When** toca el icono de menú,<br>**Then** se despliegan las opciones de forma vertical y táctil.<br><br>**Scenario:** Contenido desbordado en pantallas pequeñas.<br>**Given** que la pantalla del móvil es muy angosta,<br>**When** carga la tabla de precios,<br>**Then** el sistema permite un desplazamiento horizontal suave para ver todas las columnas sin romper el diseño. | EP-01 |
| **US-14** | Creación de perfil de condominio | **Como** administrador, **quiero** registrar un nuevo edificio en el sistema, **para** empezar a gestionar sus finanzas de manera independiente. | **Scenario:** Registro de edificio completo.<br>**Given** que el administrador ingresa datos válidos,<br>**When** presiona guardar,<br>**Then** el sistema crea el condominio y habilita su panel de control.<br><br>**Scenario:** Registro duplicado.<br>**Given** que un administrador intenta registrar un edificio con el mismo nombre y dirección que uno ya existente,<br>**When** intenta guardar,<br>**Then** el sistema muestra una alerta indicando que el condominio ya está registrado. | EP-02 |
| **US-15** | Importación masiva de residentes | **Como** administrador, **quiero** subir un archivo Excel con los datos de los vecinos, **para** no tener que registrar departamento por departamento manualmente. | **Scenario:** Importación de lista limpia.<br>**Given** un archivo Excel con 50 residentes,<br>**When** se carga al sistema,<br>**Then** se crean las 50 cuentas y se envían los accesos.<br><br>**Scenario:** Archivo con formato incorrecto.<br>**Given** un archivo con columnas cambiadas o datos vacíos,<br>**When** se intenta procesar,<br>**Then** el sistema rechaza la carga y descarga un informe con los errores encontrados. | EP-02 |
| **US-16** | Generación masiva de recibos | **Como** administrador, **quiero** generar los recibos de mantenimiento de todos los departamentos con un solo clic, **para** ahorrar tiempo al inicio de cada mes. | **Scenario:** Emisión mensual exitosa.<br>**Given** que se ingresaron los gastos del mes,<br>**When** se genera el proceso masivo,<br>**Then** se crean recibos 'Pendientes' para todos los residentes.<br><br>**Scenario:** Generación sin gastos base.<br>**Given** que el administrador intenta generar recibos sin haber configurado el presupuesto del mes,<br>**When** hace clic en generar,<br>**Then** el sistema muestra un error indicando que falta información de costos. | EP-03 |
| **US-17** | Pago de mantenimiento en línea | **Como** residente, **quiero** pagar mi cuota de mantenimiento usando mi tarjeta desde la plataforma, **para** evitar hacer transferencias bancarias manuales. | **Scenario:** Pago con tarjeta aprobado.<br>**Given** que el residente ingresa datos de tarjeta válidos,<br>**When** confirma el pago,<br>**Then** el recibo cambia a 'Pagado' inmediatamente.<br><br>**Scenario:** Tarjeta rechazada por fondos insuficientes.<br>**Given** que la tarjeta no tiene saldo,<br>**When** se intenta procesar el pago,<br>**Then** la pasarela devuelve un error y el recibo permanece como 'Pendiente'. | EP-04 |
| **US-18** | Conciliación automática del pago | **Como** administrador, **quiero** que el sistema marque automáticamente como "Pagado" un recibo abonado por la web, **para** evitar cruzar vouchers manualmente. | **Scenario:** Conciliación inmediata post-pago.<br>**Given** una notificación exitosa de la pasarela,<br>**When** el ID del recibo coincide,<br>**Then** el sistema liquida la deuda en el dashboard financiero.<br><br>**Scenario:** Notificación de pago demorada.<br>**Given** que el pago fue exitoso en el banco pero la notificación al sistema falla,<br>**When** pasan 24 horas sin actualización,<br>**Then** el sistema permite al administrador cargar el comprobante manualmente para validación. | EP-04 |
| **US-19** | Aplicación automática de mora | **Como** administrador, **quiero** que el sistema aplique una multa automática a los recibos vencidos, **para** penalizar la morosidad sin tener que discutir con el vecino. | **Scenario:** Recargo por vencimiento.<br>**Given** un recibo vencido el día de hoy,<br>**When** se ejecuta la tarea nocturna,<br>**Then** el monto de la mora se añade al total del recibo.<br><br>**Scenario:** Error en la tasa de interés.<br>**Given** que la tasa de mora está configurada en 0%,<br>**When** llega la fecha de vencimiento,<br>**Then** el sistema no realiza cambios en el monto y notifica al administrador para revisar la configuración. | EP-05 |
| **US-20** | Reserva de áreas comunes (Parrilla) | **Como** residente, **quiero** reservar la zona de parrillas desde la plataforma, **para** asegurar mi turno sin buscar al conserje. | **Scenario:** Reserva de horario libre.<br>**Given** que el horario está disponible,<br>**When** el residente confirma,<br>**Then** el horario queda bloqueado y confirmado.<br><br>**Scenario:** Intento de reserva en horario ocupado.<br>**Given** que un vecino ya reservó el turno de las 8:00 PM,<br>**When** otro residente intenta elegir el mismo bloque,<br>**Then** el sistema muestra el horario en gris y bloquea el botón de reserva. | EP-06 |
| **US-21** | Generación de código QR para visitas | **Como** residente, **quiero** generar un código QR de invitación, **para** que mis visitas puedan ingresar rápido y el conserje los valide. | **Scenario:** QR generado con éxito.<br>**Given** datos del invitado válidos,<br>**When** se genera el código,<br>**Then** se descarga una imagen lista para compartir.<br><br>**Scenario:** QR expirado.<br>**Given** un código generado hace más de 24 horas,<br>**When** el conserje intenta escanearlo,<br>**Then** el lector muestra un mensaje de "Acceso Denegado: Código Expirado". | EP-06 |
| **US-22** | Visualización del Dashboard financiero | **Como** administrador, **quiero** ver un resumen visual de los ingresos y el porcentaje de morosidad, **para** conocer la salud financiera del edificio rápidamente. | **Scenario:** Carga de gráficos en tiempo real.<br>**Given** que existen pagos y deudas en el mes,<br>**When** se abre el dashboard,<br>**Then** se muestran gráficos de barras y pasteles actualizados.<br><br>**Scenario:** Dashboard sin datos previos.<br>**Given** un edificio recién creado,<br>**When** el administrador abre el dashboard,<br>**Then** el sistema muestra indicadores en cero con un mensaje invitando a realizar la primera facturación. | EP-07 |
